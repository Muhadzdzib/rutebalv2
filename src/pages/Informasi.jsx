...
export default function Informasi() {
  ...
  // ====== Label Name ======
  const prettyNames = {
    Rute_busA1: "Bus Koridor A1",
    Rute_busA2: "Bus Koridor A2",
    Rute_busB1: "Bus Koridor B1",
    Rute_busB2: "Bus Koridor B2",
    Rute_Trayek1A: "Angkot 1A",
    Rute_Trayek1B: "Angkot 1B",
    Rute_Trayek2A: "Angkot 2A",
    Rute_Trayek2B: "Angkot 2B",
    Rute_Trayek3A: "Angkot 3A",
    Rute_Trayek3B: "Angkot 3B",
    Rute_Trayek5A: "Angkot 5A",
    Rute_Trayek5B: "Angkot 5B",
    Rute_Trayek6A: "Angkot 6A",
    Rute_Trayek6B: "Angkot 6B",
    Rute_Trayek7A: "Angkot 7A",
    Rute_Trayek7B: "Angkot 7B",
    Rute_Trayek8A: "Angkot 8A",
    Rute_Trayek8B: "Angkot 8B",
  };
  ...
  useEffect(() => {
    ...
    L.marker([userLat, userLng], { icon: icons.here, title: "Lokasi Anda" })
      .addTo(leafletMap.current)
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        setSelectedJenis([...new Set(jenisList)]);
      } catch (e) {
  ...
  const renderRoutes = (filtered) => {
    clearMap();
    const sortedRoutes = [...allRoutes]; // kamu bisa pakai prioritas kalau perlu
    sortedRoutes.forEach((route) => {
      const jenisRel = route.relationship?.jenis;
      if (!jenisRel || jenisRel === "jalan_kaki" || jenisRel === "transisi")
        return;

      const cocok = filtered.includes(jenisRel);
      if (!cocok) return;
      ...
      if (route.source?.jenis !== "titik") {
        ...
        m.bindPopup(
          `<b>${route.source.nama}</b><br/>Jenis: ${route.source.jenis}`
        );
      ...
      if (route.target?.jenis !== "titik") {
        ...
        m.bindPopup(
          `<b>${route.target.nama}</b><br/>Jenis: ${route.target.jenis}`
        );
        markersRef.current.push(m);
      }
      ...
      const line = L.polyline(latlngs, {
        color: getColorByJenis(jenisRel), // <<< pakai fungsi
        weight: 5,
        opacity: 1,
      }).addTo(leafletMap.current);
      ...
      <div className="relative z-10 w-full flex flex-col md:flex-row mx-[10%] gap-6">
        ...
        <div className="w-full md:w-1/3 text-left text-white">
          ...
          <div className="bg-white/5 backdrop-blur-md border border-white/10 rounded-xl p-4 max-h-[560px] overflow-y-auto">
            <p className="text-sm text-white/80 mb-3">Pilih Jenis Rute:</p>
            <div className="grid grid-cols-2 gap-x-3 gap-y-2">
              {uniqueJenis.map((jenis) => (
                ...
                >
                  ...
                  />
                  <span className="text-[13px] md:text-sm font-medium text-white/90 leading-snug">
                    {prettyNames[jenis] || jenis}
                  </span>
                </label>
              ))}
